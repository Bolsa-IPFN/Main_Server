...
function getPoints()
{
	var url = 'http://' + rpiIP + '/resultpoint?name=Arduino_Temp';
	var dados = {}
	$.ajax({
      url: url,      //Your api url
      ...
      success: function (response) {
		...
		if (response.status !== 'undefined' && response.status === 'Experiment Ended')
		{
			...
			if (typeof response.Data === 'object')
			{
				...
				Plotly.extendTraces('graph', {x: Array(6).fill([response.Data.sample]),y: [[response.Data.temp],[response.Data.temp_bot],[response.Data.temp_in],[response.Data.temp_north],[response.Data.temp_south],[response.Data.temp_top]]}, [0,1,2,3,4,5]);
				// cnt++;
				// if (cnt >120){
...
//   Results = setInterval(getPoints,300)
